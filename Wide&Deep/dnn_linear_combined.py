  ...
  def __init__(self,
    ...
    if not self._feature_columns:
      raise ValueError('Either linear_feature_columns or dnn_feature_columns '
                       'must be defined.')
    # LOSS FUNCTION
    def logistic_loss(labels, logits):
    ...
    def focal_loss(labels, logits):
      ...
      gamma = array_ops.constant(gamma, dtype = 'float32', shape=shape)
      ones = array_ops.ones(shape=shape)
      # loss = (1-probs)^gamma*labels*log(1+exp(-logits)) + probs^gamma*(1-y)*log(1+exp(-logits))
      zeros = array_ops.zeros_like(logits, dtype=logits.dtype)
      cond = (logits >= zeros)
      relu_logits = array_ops.where(cond, logits, zeros)
      reversed_relu_logits = array_ops.where(cond, zeros, logits)
      neg_abs_logits = array_ops.where(cond, -logits, logits)
      return math_ops.add(
          math_ops.pow(math_ops.subtract(ones, probs), gamma) *
          labels *
          math_ops.subtract(math_ops.log1p(math_ops.exp(neg_abs_logits)), reversed_relu_logits),
          math_ops.pow(probs, gamma) *
          math_ops.subtract(ones, labels) *
          math_ops.add(relu_logits, math_ops.log1p(math_ops.exp(neg_abs_logits)))
          )
  
    def wighted_logistic_loss(labels, logits):
      logits = ops.convert_to_tensor(logits)
      labels = ops.convert_to_tensor(labels)
      try:
        labels.get_shape().merge_with(logits.get_shape())
    ...
    if n_classes == 2:
      loss_function = None
      if CONF.loss_fn == 'log_loss':
        loss_function = logistic_loss
      elif CONF.loss_fn == 'focal_loss':
        loss_function = focal_loss
      elif CONF.loss_fn == 'weighted_log_loss':
        loss_function = wighted_logistic_loss
